-- [nfnl] fnl/nix-update/utils/fp/option.fnl
local Option = {}
Option["some?"] = function(mx)
  if ((_G.type(mx) == "table") and (mx[1] == "some")) then
...
Option["none?"] = function(mx)
  if ((_G.type(mx) == "table") and (mx[1] == "none")) then
    return true
  else
    local _ = mx
    return false
...
Option["option?"] = function(mx)
  return (Option["some?"](mx) or Option["none?"](mx))
end
...
Option[">>="] = function(mx, f)
  if ((_G.type(mx) == "table") and (mx[1] == "some")) then
    local x = {select(2, (table.unpack or _G.unpack)(mx))}
    return f(unpack(x))
  else
    local _ = mx
    return mx
...
Option.unwrap = function(mx)
  if ((_G.type(mx) == "table") and (mx[1] == "some")) then
    local x = {select(2, (table.unpack or _G.unpack)(mx))}
    return unpack(x)
  elseif ((_G.type(mx) == "table") and (mx[1] == "none")) then
    return nil
  else
    return nil
  end
...
Option["unwrap!"] = function(mx)
  if ((_G.type(mx) == "table") and (mx[1] == "some")) then
    ...
    return unpack(x)
  elseif ((_G.type(mx) == "table") and (mx[1] == "none")) then
    return error("Tried to unwrap a none")
  else
    return nil
  end
end
