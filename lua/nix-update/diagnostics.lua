-- [nfnl] fnl/nix-update/diagnostics.fnl
...
local _local_3_ = require("nix-update.utils")
...
local function set_diagnostic(opts)
  ...
  if (err and (#(data or {}) == 0)) then
    do
      local _let_6_ = coords({bufnr = bufnr, node = fetch._fwhole})
      local start_row = _let_6_["start-row"]
      local start_col = _let_6_["start-col"]
      vim.diagnostic.set(namespace, bufnr, {{lnum = start_row, col = start_col, severity = vim.diagnostic.severity.ERROR, message = vim.inspect(err), source = "NixUpdate"}})
    end
    return nil
  else
  end
      ...
      do
        local function _8_()
          ...
          if farg then
            ...
            local start_row = _let_9_["start-row"]
            local start_col = _let_9_["start-col"]
            return {["start-row"] = start_row, ["start-col"] = start_col, message = string.format("Update field \"%s\" to \"%s\"", key, value), severity = vim.diagnostic.severity.HINT}
          else
            local _let_10_ = coords({bufnr = bufnr, node = fetch._fwhole})
            local start_row = _let_10_["start-row"]
            local start_col = _let_10_["start-col"]
        ...
        local severity = _let_12_.severity
        val_28_ = {lnum = start_row, col = start_col, severity = severity, message = message, source = "NixUpdate"}
      end
      if (nil ~= val_28_) then
        i_27_ = (i_27_ + 1)
  ...
  return vim.diagnostic.set(namespace, bufnr, vim.iter(ipairs(vim.diagnostic.get(nil, {namespace = namespace}))):chain(vim.iter(ipairs(diagnostics))):totable())
end
...
local function NixPrefetch(opts)
  ...
  for _, fetch in ipairs(found_fetches) do
    prefetch_fetch({bufnr = bufnr0, fetch = fetch})
  end
  return nil
end
local function _14_()
