...
assert_truthy((vim.async and vim.async.run and vim.async.await), "vim.async is required for checks")
...
local function fn_2_(value)
  return (value == 1)
end
assert_truthy((fp.Result.unwrap(fp.Result.validate(fp.Result.ok(1), fn_2_, "wrong value")) == 1), "Result.validate did not preserve Ok")
local function fn_3_(_)
  return false
end
assert_truthy(fp.Result["err?"](fp.Result.validate(fp.Result.ok(1), fn_3_, "wrong value")), "Result.validate did not construct Err")
local do_tgt_5_
do
  local t_4_ = nix_update.config
  if (nil ~= t_4_) then
    t_4_ = t_4_["update-actions"]
  else
  end
  if (nil ~= t_4_) then
    t_4_ = t_4_[1]
...
assert_truthy((do_tgt_5_ == "preview"), "vim.g.nix_update did not apply")
nix_update.setup({update_actions = {"apply", "notify"}})
local do_tgt_9_
do
  local t_8_ = nix_update.config
  if (nil ~= t_8_) then
    t_8_ = t_8_["update-actions"]
  else
  end
  if (nil ~= t_8_) then
    t_8_ = t_8_[1]
...
assert_truthy((do_tgt_9_ == "apply"), "setup overrides did not apply")
local _local_12_ = require("nix-update.utils")
local call_command = _local_12_["call-command"]
local command_task
local function hashfn_13_()
  local _let_14_ = call_command({cmd = "printf", args = {"async-ok\\n"}})
  local stdout = _let_14_.stdout
  return assert_truthy((table.concat(stdout) == "async-ok"), "vim.async command prefetch did not finish")
end
command_task = vim.async.run(hashfn_13_)
command_task:wait(1000)
...
if nix_parser then
  ...
  local function assert_lua_prefetch(label, runner, expected)
    ...
    local function hashfn_18_()
      return (cache.cache[fetch._fwhole] ~= nil)
    end
    assert_truthy(vim.wait(1000, hashfn_18_), (label .. ": prefetch did not finish"))
    return assert_truthy((cache.cache[fetch._fwhole].data.hash == expected), (label .. ": wrong hash"))
  end
  local function fn_19_(args)
    assert_truthy((args.url == "https://example.com"), "synchronous Lua prefetcher: wrong args")
    return {hash = "sync-hash"}
  end
  assert_lua_prefetch("synchronous Lua prefetcher", fn_19_, "sync-hash")
  local function fn_20_(args, done)
    assert_truthy((args.url == "https://example.com"), "asynchronous Lua prefetcher: wrong args")
    local function hashfn_21_()
      return done({hash = "async-hash"})
    end
    return vim.schedule(hashfn_21_)
  end
  assert_lua_prefetch("asynchronous Lua prefetcher", fn_20_, "async-hash")
else
end
