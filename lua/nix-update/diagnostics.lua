-- [nfnl] fnl/nix-update/diagnostics.fnl
 ...
 local diagnostics
 do local tbl_26_ = {} local i_27_ = 0 for key, value in pairs(data) do local val_28_
 do



 local function do_iife_8_() local farg = fetch._fargs[key]
 if farg then
 ...
 return {["start-row"] = start_row, ["start-col"] = start_col, message = string.format("Add new field \"%s\" with value \"%s\"", key, value), severity = vim.diagnostic.severity.WARN} end end local _let_12_ = do_iife_8_() local start_row = _let_12_["start-row"] local start_col = _let_12_["start-col"] local message = _let_12_.message local severity = _let_12_.severity
 ...
 val_28_ = {lnum = start_row, col = start_col, severity = severity, message = message, source = "NixUpdate"} end if (nil ~= val_28_) then i_27_ = (i_27_ + 1) tbl_26_[i_27_] = val_28_ else end end diagnostics = tbl_26_ end
 ...
 return vim.diagnostic.set(namespace, bufnr, vim.iter(ipairs(vim.diagnostic.get(nil, {namespace = namespace}))):chain(vim.iter(ipairs(diagnostics))):totable()) end
