-- [nfnl] fnl/nix-update/utils/fp/result.fnl
 ...
 return {"err", ...} end
 Result.new = function(...)
 local case_3_, case_4_ = ... local and_5_ = (nil ~= case_3_) if and_5_ then local r = case_3_ and_5_ = Result["result?"](r) end if and_5_ then local r = case_3_
 return r elseif ((case_3_ == nil) and (nil ~= case_4_)) then local err = case_4_
 return Result.err(err) else local _ = case_3_
 ...
 local _3e_3d_3d_13_ = Result[">>="] local pure_14_ = Result.pure local function fn_15_(x)

 if p(x) then
 return Result.new(x) else
 return Result.err(e) end end return _3e_3d_3d_13_(v, fn_15_) end
