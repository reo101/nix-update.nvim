-- [nfnl] fnl/nix-update/utils/common.fnl
 ...
 local tbl_26_ = {} local i_27_ = 0 for child, _3fname in node:iter_children() do local val_28_
 if p_3f(child, _3fname) then
 val_28_ = child else val_28_ = nil end if (nil ~= val_28_) then i_27_ = (i_27_ + 1) tbl_26_[i_27_] = val_28_ else end end return tbl_26_ end
 ...
 local function fn_4_(k, _) return k end tbl_keys = vim.iter(pairs(tbl)):map(fn_4_):totable()
 ...
 local function fn_6_(_0, k) return vim.list_contains(tbl_keys, k) end found = vim.iter(ipairs(key)):any(fn_6_)
 if not found then
 return {["any-of"] = key} else return nil end elseif not vim.list_contains(tbl_keys, key) then


 return {required = key} else return nil end end return vim.iter(ipairs(required_keys)):map(fn_5_):totable() end
 ...
 recurse(v) end return nil else

 return table.insert(result, t) end end
 recurse(tbl)
 return result end
