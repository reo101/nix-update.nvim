...
local function gen_fetches_names()
  ...
  do
    local t_6_ = config
    if (nil ~= t_6_) then
      t_6_ = t_6_["extra-prefetchers"]
    else
    end
    _7_ = t_6_
  end
  vim.list_extend(names, keys(_7_()))
...
local function get_root(opts)
  ...
  local _let_10_ = parser:parse()
  local tree = _let_10_[1]
  return tree:root()
end
  ...
  for binding, _ in bounder:iter_children() do
    ...
    if (case_13_ == "binding") then
      ...
      do
        ...
        bool_expr0 = find_child(binding, _16_)
        if bool_expr0 then
      ...
      do
        ...
        string_expression = find_child(binding, _19_)
        if string_expression then
          if (string_expression:named_child_count() > 0) then
            ...
            for node, _0 in string_expression:iter_children() do
              local val_28_
              do
                local case_20_ = node:type()
                if (case_20_ == "interpolation") then
                  local _21_
                  do
                    ...
                    if (nil ~= node) then
                      ...
                      variable_expression = find_child(node, _22_)
                    else
                      variable_expression = nil
                    end
                    if variable_expression then
                      _21_ = {["?interp"] = node, name = vim.treesitter.get_node_text(variable_expression, bufnr0)}
                    else
                      _21_ = nil
                    end
                  end
                  ...
                  if not or_25_ then
                    ...
                    if (nil ~= node) then
                      ...
                      select_expression = find_child(node, _27_)
                    else
                    ...
                    if (nil ~= select_expression) then
                      ...
                      tmp_3_ = find_child(select_expression, _29_)
                      if (nil ~= tmp_3_) then
                    ...
                    if (nil ~= select_expression) then
                      ...
                      tmp_3_ = find_child(select_expression, _32_)
                      if (nil ~= tmp_3_) then
                    ...
                    if (attrset_name and attr_name0) then
                      or_25_ = {["?interp"] = node, name = attr_name0, ["?from"] = attrset_name}
                    else
                      or_25_ = nil
                    end
                  end
                  ...
                  local and_36_ = (nil ~= case_20_)
                  if and_36_ then
                    local t = case_20_
                    and_36_ = ((t == "string_fragment") or (t == "escape_sequence"))
                  end
                  if and_36_ then
                    local t = case_20_
            ...
            local _let_40_ = coords({bufnr = bufnr0, node = string_expression})
            local start_row = _let_40_["start-row"]
            local start_col = _let_40_["start-col"]
      ...
      do
        ...
        if (nil ~= binding) then
          ...
          variable_expression = find_child(binding, _43_)
        else
          variable_expression = nil
        end
        if variable_expression then
      ...
      do
        ...
        if (nil ~= binding) then
          ...
          select_expression = find_child(binding, _46_)
        else
          select_expression = nil
        end
        local attrset_name
        if (nil ~= select_expression) then
          ...
          tmp_3_ = find_child(select_expression, _48_)
          if (nil ~= tmp_3_) then
            attrset_name = vim.treesitter.get_node_text(tmp_3_, bufnr0)
          else
            attrset_name = nil
        ...
        if (nil ~= select_expression) then
          ...
          tmp_3_ = find_child(select_expression, _51_)
          if (nil ~= tmp_3_) then
            attr_name0 = vim.treesitter.get_node_text(tmp_3_, bufnr0)
          else
            attr_name0 = nil
...
local function try_get_binding_value(opts)
  ...
  local recurse_3f
  do
    local case_61_ = bounder:parent():type()
    if (case_61_ == "attrset_expression") then
      recurse_3f = false
    elseif ((case_61_ == "let_expression") or (case_61_ == "rec_attrset_expression")) then
      recurse_3f = true
    else
      recurse_3f = nil
    end
  ...
  local function find_parent_bounder()
    ...
    while true do
      local and_67_ = parent_bounder
      if and_67_ then
        local and_68_ = (parent_bounder:type() ~= "rec_attrset_expression") and (parent_bounder:type() ~= "let_expression")
        if and_68_ then
          local and_69_ = (parent_bounder:type() == "attrset_expression")
          if and_69_ then
            local tmp_3_ = parent_bounder:parent()
            if (nil ~= tmp_3_) then
              local tmp_3_0 = tmp_3_:type()
              if (nil ~= tmp_3_0) then
                and_69_ = (tmp_3_0 == "function_expression")
              else
                and_69_ = nil
              end
            else
        ...
        local or_74_ = and_68_
        if not or_74_ then
          local function _75_(_241)
            return (_241:type() == "binding_set")
          end
          or_74_ = not find_child(parent_bounder, _75_)
        end
        and_67_ = or_74_
      end
      if not and_67_ then break end
    ...
    if (nil ~= parent_bounder) then
      ...
      if (nil ~= tmp_3_) then
        _76_ = (tmp_3_ == "attrset_expression")
      else
        _76_ = nil
      end
    else
    ...
    if and_80_ then
      ...
      universal_parameter = find_child(parent, _86_)
      local formals
      local _87_
      if (nil ~= parent) then
        local tmp_3_
        ...
        tmp_3_ = find_child(parent, _89_)
        if (nil ~= tmp_3_) then
          local tmp_3_0
          local function _91_(_241, _242)
            local and_92_ = (_241:type() == "formal") and (_242 == "formal")
            if and_92_ then
              local function _93_(_2410, _2420)
                return (_2420 == "default")
              end
              and_92_ = (find_child(_241, _93_) == nil)
            end
            return and_92_
          end
          tmp_3_0 = find_children(tmp_3_, _91_)
          if (nil ~= tmp_3_0) then
            local tmp_3_1 = vim.iter(tmp_3_0)
            if (nil ~= tmp_3_1) then
              local tmp_3_2
              local function _96_(_241)
                local function _97_(_2410, _2420)
                  return (_2420 == "name")
                end
                return find_child(_241, _97_)
              end
              tmp_3_2 = tmp_3_1:map(_96_)
              if (nil ~= tmp_3_2) then
                local tmp_3_3
                local function _99_(_241)
                  return vim.treesitter.get_node_text(_241, bufnr0)
                end
                tmp_3_3 = tmp_3_2:map(_99_)
                if (nil ~= tmp_3_3) then
                  _87_ = tmp_3_3:totable()
                else
                  _87_ = nil
                end
              else
    ...
    if parent_bounder then
      ...
      parent_bounder = find_child(parent_bounder, _108_)
    else
    end
  ...
  if binding then
    ...
    local function _111_(_110_)
      ...
      elseif ((_G.type(fragment) == "table") and true and (nil ~= fragment.name) and true) then
        ...
        local _let_112_ = find_parent_bounder()
        local next_from = _let_112_.from
        local only_for = _let_112_["only-for"]
        local parent_bounder = _let_112_["parent-bounder"]
        local next_bounder
    ...
    local _let_119_ = find_parent_bounder()
    local parent_bounder = _let_119_["parent-bounder"]
    local from0 = _let_119_.from
    if parent_bounder then
...
local function try_get_binding_bounder(opts)
  ...
  do
    ...
    for binding, _ in node:iter_children() do
      local val_28_
      do
        local case_125_ = binding:type()
        if (case_125_ == "binding") then
          local attr
          local function _126_(_241)
            return (_241:type() == "attrpath")
          end
          attr = find_child(binding, _126_)
          local attr_name
          if attr then
            ...
            do
              ...
              local function _137_()
                ...
                elseif (capture_id == "_fargs") then
                  local tbl_21_0 = {}
                  for name, _0 in pairs(find_all_local_bindings({bufnr = bufnr0, bounder = node})) do
                    local k_22_0, v_23_0
                    do
...
local function get_fetch_at_cursor(opts)
  ...
  local _local_140_ = vim.fn.getcursorcharpos()
  local _ = _local_140_[1]
  local cursor_row = _local_140_[2]
  local cursor_col = _local_140_[3]
  local _0 = _local_140_[4]
  local _1 = _local_140_[5]
    ...
    if existing then
      ...
      while (not short_circuit_3f and (i_new_value <= #new_value)) do
        ...
        elseif (i_fragment == #existing) then
          local _local_146_ = coords({bufnr = bufnr, node = fragment_node})
          local start_row = _local_146_["start-row"]
          local start_col = _local_146_["start-col"]
          local end_row = _local_146_["end-row"]
          local end_col = _local_146_["end-col"]
          ...
          local _local_147_ = coords({bufnr = bufnr, node = (fragment__3finterp or fragment_node)})
          local start_row = _local_147_["start-row"]
          local start_col = _local_147_["start-col"]
          local _local_148_ = coords({bufnr = bufnr, node = (last_fragment__3finterp or last_fragment_node)})
          local end_row = _local_148_["end-row"]
          local end_col = _local_148_["end-col"]
          table.insert(updates, {type = "old", data = {bufnr = bufnr, ["start-row"] = start_row, ["start-col"] = start_col, ["end-row"] = end_row, ["end-col"] = end_col, replacement = {string.sub(new_value, i_new_value)}}})
          short_circuit_3f = true
        end
      end
    else
      local _let_150_ = coords({bufnr = bufnr, node = fetch._fwhole})
      local end_row = _let_150_["end-row"]
      local end_col = _let_150_["end-col"]
...
local function preview_update(update)
  ...
  if ((_G.type(update) == "table") and (update.type == "old") and ((_G.type(update.data) == "table") and (nil ~= update.data.bufnr) and (nil ~= update.data["start-row"]) and (nil ~= update.data["start-col"]) and (nil ~= update.data["end-row"]) and (nil ~= update.data["end-col"]) and (nil ~= update.data.replacement))) then
    ...
    return vim.api.nvim_buf_set_extmark(bufnr, namespace, start_row, start_col, {end_row = end_row, end_col = end_col, hl_mode = "replace", virt_text = _152_, virt_text_pos = "overlay"})
  elseif ((_G.type(update) == "table") and (update.type == "new") and ((_G.type(update.data) == "table") and (nil ~= update.data.bufnr) and (nil ~= update.data.start) and (nil ~= update.data.replacement))) then
    ...
    return vim.api.nvim_buf_set_extmark(bufnr, namespace, start, 0, {virt_lines = _154_, virt_lines_above = true})
  else
    return nil
  end
end
...
local function notify_update(update)
  ...
  elseif ((_G.type(update) == "table") and (update.type == "new") and ((_G.type(update.data) == "table") and (nil ~= update.data.bufnr) and (nil ~= update.data.start) and (nil ~= update.data["end"]) and (nil ~= update.data.replacement))) then
    ...
    local _end = update.data["end"]
    local replacement = update.data.replacement
    return vim.notify(string.format("Inserted text at (%d, %d) to (%d, %d) in buffer %d with %s", __fnl_global__start_2drow, __fnl_global__start_2dcol, __fnl_global__end_2drow, __fnl_global__end_2dcol, bufnr, vim.inspect(replacement)))
  else
    return nil
  end
end
local function prefetch_fetch(opts)
  ...
  do
    local t_160_ = config
    if (nil ~= t_160_) then
      t_160_ = t_160_["extra-prefetchers"]
    else
    end
    if (nil ~= t_160_) then
      t_160_ = t_160_[fetch0._fname]
    else
    end
    _161_ = t_160_
  end
  local or_164_ = _161_
  if not or_164_ then
    local t_165_ = prefetchers
    if (nil ~= t_165_) then
      t_165_ = t_165_[fetch0._fname]
    else
    end
    or_164_ = t_165_
  end
  ...
  do
    ...
    for farg_name, farg_binding in pairs(fetch0._fargs) do
      ...
      Result.bimap(fragments_to_value(farg_binding.fragments), _168_, _169_)
    end
    for _, _170_ in ipairs(notfounds_pairs) do
      local farg_name = _170_["farg-name"]
      local notfounds = _170_.notfounds
  ...
  local function _174_(_173_)
    local stdout = _173_.stdout
    local stderr = _173_.stderr
    if (#stdout == 0) then
  ...
  call_command(prefetcher_cmd, _174_)
