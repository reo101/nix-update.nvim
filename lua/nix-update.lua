-- [nfnl] fnl/nix-update.fnl
 local _local_1_ = require("nix-update._cache") local cache = _local_1_.cache


 local _local_2_ = require("nix-update._config") local config = _local_2_.config
 local apply_options = _local_2_["apply-options"] local initialized_3f = false local global_options_applied_3f = false
 ...
 local tbl_21_ = {} for k, v in pairs((opts or {})) do local k_22_, v_23_
 if (type(k) == "string") then
 k_22_, v_23_ = string.gsub(k, "_", "-"), v else
 k_22_, v_23_ = k, v end if ((k_22_ ~= nil) and (v_23_ ~= nil)) then tbl_21_[k_22_] = v_23_ else end end return tbl_21_ end
 ...
 normalized_global_options = normalize_options(global_options) else
 normalized_global_options = {} end

 local ok, err = apply_options(normalized_global_options)
 if not ok then
 vim.notify(string.format("nix-update: invalid configuration: %s", err), vim.log.levels.ERROR) else end
 ...
 apply_current_options() global_options_applied_3f = true
 ...
 return true else end

 local _local_11_ = require("nix-update.fetches") local calculate_updates = _local_11_["calculate-updates"]
 ...
 local handler do local t_13_ = action_handlers if (nil ~= t_13_) then t_13_ = t_13_[action] else end handler = t_13_ end
 if handler then
 handler(update) else end end end return nil end



 local function fn_16_(new, _key, value)
 if new then
 return on_cache_write(value) else return nil end end cache({handler = fn_16_}) initialized_3f = true return true end
 ...
 vim.g.nix_update = merged global_options_applied_3f = false
 ...
 initialize()
 local _let_18_ = require("nix-update.fetches") local prefetch_fetch0 = _let_18_["prefetch-fetch"]
 return prefetch_fetch0(opts) end

 local function prefetch_buffer(opts)
 initialize()
 local _let_19_ = require("nix-update.diagnostics") local prefetch_buffer0 = _let_19_["prefetch-buffer"]
 return prefetch_buffer0(opts) end
