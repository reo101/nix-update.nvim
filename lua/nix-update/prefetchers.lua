...
local function fn_7_(arg_6_)
  local owner = arg_6_.owner
  local repo = arg_6_.repo
  local rev = arg_6_.rev
  local tag = arg_6_.tag
  local fetchSubmodules = arg_6_.fetchSubmodules
  local cmd = "nurl"
  local ref = (rev or tag)
...
local function fn_9_(arg_8_)
  local url = arg_8_.url
  local name = arg_8_.name
  local cmd = "nix"
  local args
  local if_tgt_10_
  if name then
    if_tgt_10_ = {"--name", name}
  else
    if_tgt_10_ = {}
  end
  args = vim.iter({{"store"}, {"prefetch-file"}, {"--json"}, {"--hash-type", "sha256"}, if_tgt_10_, {url}}):flatten():totable()
  return {cmd = cmd, args = args}
end
local function fn_13_(arg_12_)
  local url = arg_12_.url
  local cmd = "nix"
  local args = {"store", "prefetch-file", "--json", "--hash-type", "sha256", url}
  return {cmd = cmd, args = args}
end
local function fn_15_(arg_14_)
  local url = arg_14_.url
  local rev = arg_14_.rev
  local fetchSubmodules = arg_14_.fetchSubmodules
  local cmd = "nurl"
  local args = {"--json", "--fetcher", "builtins.fetchGit", string.format("--submodules=%s", (fetchSubmodules or "false")), url, rev}
  return {cmd = cmd, args = args}
end
prefetchers = {fetchFromGitHub = {["required-cmds"] = {"nurl"}, ["required-keys"] = {"owner", "repo", {"tag", "rev"}}, prefetcher = fn_5_, extractor = nurl_json_hash_extractor}, fetchFromGitLab = {["required-cmds"] = {"nurl"}, ["required-keys"] = {"owner", "repo", {"tag", "rev"}}, prefetcher = fn_7_, extractor = nurl_json_hash_extractor}, fetchurl = {["required-cmds"] = {"nix"}, ["required-keys"] = {"url"}, prefetcher = fn_9_, extractor = nix_json_hash_extractor}, fetchpatch = {["required-cmds"] = {"nix"}, ["required-keys"] = {"url"}, prefetcher = fn_13_, extractor = nix_json_hash_extractor}, fetchgit = {["required-cmds"] = {"nurl"}, ["required-keys"] = {"url", "rev"}, prefetcher = fn_15_, extractor = nurl_json_hash_extractor}}
